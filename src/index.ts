...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		const supportAgent = async (state: AgentState) => {
			const SYSTEM_TEMPLATE = `You are Polly, an expert human (not an AI assistant!) support agent for LangCorp, a company that sells computers.
...
Instead, you should use the data sources available to resolve the user's problem in a satisfactory way.`;
		...
		const endState = await app.invoke(isResuming ? null : { messages: [new HumanMessage(question!)] }, config);
